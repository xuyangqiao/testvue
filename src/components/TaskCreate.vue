<template>
    <div class="content-wrapper">
                ...
                <el-row>
                    <el-col :span="10">
                        <el-form ref="form" :model="form" label-width="100px">
                            ...
                            <el-form-item :label="$lang('发包类型:')" required>
                            ...
                            <el-form-item :label="$lang('任务组:')">
                                ...
                                <el-col :span="4" :push="1" style="white-space: nowrap;">
                                    <a href="javascript:;" class="new-group-a" @click="createGroup = true">{{$lang('新建分组')}}</a>
                                    <span>|</span>
                                    <a href="javascript:;" class="new-group-a" @click="editGroup = true">{{$lang('编辑分组')}}</a>
                                </el-col>
                            </el-form-item>
                        </el-form>
                    </el-col>
                    <el-col :span="11" :offset="3">
                        <div class="new-tast-upload" style="display: none">
                            <el-upload ref="LogoUpload" class="avatar-uploader" action="https://jsonplaceholder.typicode.com/posts/" :show-file-list="false" :on-change="logoChange" :auto-upload="false">
            ...
            <div class="new-task-other">
                <el-row>
                    <el-form ref="form" :model="form1" label-width="100px">
                        <el-row>
                            <el-col :span="10">
                                <el-form-item :label="$lang('应用领域:')" required>
                                ...
                                <el-form-item :label="$lang('应用环境:')" required>
                                ...
                                <el-form-item class="zy-select" :label="$lang('应用类别:')" required>
                                    <Type-Select :onChange="typeSelectChange" :dataSource="AppClass" :value="TypeSelectValue"></Type-Select>
                                </el-form-item>
                            </el-col>
                        </el-row>

                        <el-form-item :label="$lang('参考图片:')" class="update-images-list" required>
        ...
        <el-dialog :title="$lang('新建分组')" :visible.sync="createGroup" size="tiny">
            <div class="">
        ...
        <el-dialog :title="$lang('编辑分组')" :visible.sync="editGroup" size="tiny">
            <el-table :data="taskGroup" style="width: 100%">
                <el-table-column prop="date" label="分组名称">
                    <template scope="scope">
                        <el-input v-model="scope.row.backup" v-if="scope.row.editable" />
                ...
                <el-table-column prop="name" label="操作" width="100px">
                    <template scope="scope">
                        <div v-if="scope.row.editable">
                            <a href="javascript:;" @click="saveGroup(scope.row)">保存</a>
                            <a href="javascript:;" @click="scope.row.editable=false">取消</a>
                        </div>
                        <div v-else>
                            <a href="javascript:;" @click="scope.row.backup=scope.row.groupName,scope.row.editable=true">修改</a>
                            <a href="javascript:;" @click="removeGroup(scope.row)">删除</a>
                        </div>
                    </template>
                </el-table-column>
            </el-table>
...
export default {
  ...
  async mounted() {
    const me = this;
    const { areaList, enviList, taskGroup, AppClass } = JSON.parse(
      localStorage.getItem("TaskParam") || {}
    );
    ...
    this.taskGroup =
      taskGroup &&
      taskGroup.map(o =>
        Object.assign(o, { editable: false, backup: o.groupName })
      );
  ...
  methods: {
    async saveGroup(o) {
      let res = await saveGroup(o.id, o.backup);
      if (res.success) {
        this.taskGroup = this.taskGroup || [];
        this.taskGroup.forEach(
          o => o.id == res.data.id && Object.assign(o, res.data)
        );
        let taskParam = JSON.parse(localStorage.getItem("TaskParam") || "{}");
        localStorage.setItem(
          "TaskParam",
          JSON.stringify(
            Object.assign(taskParam, { taskGroup: this.taskGroup })
          )
        );
    ...
    async removeGroup(o) {
      let res = await removeGroup(o.id);
      if (res.success) {
        this.taskGroup = this.taskGroup || [];
    ...
    async uploadAllFile(id) {
      ...
      file.map((item, i) => {
        if (fileListType[i] == 3) {
          PromiseList.push(
            uploadFile(item, `/task/${id}`, "taskCreateDiv" + j++)
          );
        } else {
        ...
        data.map((item, i) => {
          ...
          fileList.push({
            ...
            findex:
              fileListType[i] == 1
                ? "cover"
                : fileListType[i] == 2 ? "referencechart" : "enclosure",
            url: item.url
              ? item.url
          ...
          if (
            !ReferencePicture.filter(o => !o.raw).length &&
            i == 0 &&
            fileListType[i] == 2
          ) {
            fileList.push({
    ...
    async removeFile(file) {
      ...
      if (file.id) {
        ...
        this.removeFileNameList.push(
          `/task/${this.$route.query.id}/${file.alias}`
        );
      }
    },
    //点击按钮提交或者保存
