    ...
    <div class="chart-wrpaper clearfix">
        <div class="chart-left">
            <div class="box-flex-media-box">
                ...
                <el-button type="sure" @click="toDetail('V_detail')">查看任务&gt;</el-button>
            </div>
                ...
                <ul class="chart-left-ul">
                    <li class="chart-left-li" v-for="(item,i) in taskStage" :key="i">
                        <div class="box-flex-media-box cl-top">
                            ...
                            <div class="upload-sc" v-else>
                                <el-button type="info" v-if="!isOnlyChat" size="small">上传文件</el-button>
                                <input class="upload-file-btn" type="file" v-if="!isOnlyChat" @change="chooseFile(i, $event.target.files[0])" />
                            </div>
                            <el-button type="info" size="small" @click="toRedirect('V_History', i)">查看记录</el-button>
                        </div>
                    ...
                    <li class="chart-left-li">
                        <div class="box-flex-media-box cl-top">
                            ...
                            <el-button type="info" size="small" @click="toRedirect('V_History', '-1')">查看记录</el-button>
                        </div>
                        <dl class="box-flex cl-info">
                            <dt>截止时间：</dt>
...
export default {
    ...
    methods: {
        chooseFile(index, file) {
            ...
            if (file) {
                ...
                if (index == me.taskStage.length) {
                    ...
                    if (me.subTask.latestVersion == "NaN" || isNaN(me.subTask.latestVersion)) {
                        me.subTask.latestVersion = 1;
                    }
                    fileName = me.subTask.projectName + '-' + '验收' + "-" + me.subTask.latestVersion;
                } else {
                    me.taskStage[index].latestVersion = parseInt(me.taskStage[index].latestVersion) + 1;
                    if (me.taskStage[index].latestVersion == "NaN" || isNaN(me.taskStage[index].latestVersion)) {
                        me.taskStage[index].latestVersion = 1;
                    }
                    fileName = me.subTask.projectName + '-' + me.taskStage[index].stageName + "-" + me.taskStage[index].latestVersion;
                }
                const fileType = file.name.slice(file.name.lastIndexOf("."));
                client.then(oss => {
                    oss.multipartUpload(`/task/${me.$route.query.id}/${me.$route.query.taskId}/${fileName}${fileType}`, file, {
                        ...
                        //保存到java服务器
                        me.addFileToServer({
                            bindid: index == me.taskStage.length ? me.subTask.id : me.taskStage[index].id,
                            findex: "enclosure",
        ...
        toRedirect(name, index) {
            ...
            const id = this.$route.query.id;
            this.$router.push({ name, query: { id: id, TaskStage: TaskStage, index: index } })
        },
        toDetail(name) {
            const id = this.$route.query.id;
        ...
        submit() {
            ...
            if (!version) {
                ...
                const barId = null;
                client.then(oss => {
                    oss.multipartUpload(`/task/${this.subTask.id}/${this.subTask.latestVersion}/${file.raw.name}`, file.raw, {
        ...
        ReleaseFileChange(file, fileList) {
            ...
            if (file) {
                const barId = null;
                client.then(oss => {
                    oss.multipartUpload(`/task/${me.subTask.id}/${me.subTask.latestVersion}/Release/${file.raw.name}`, file.raw, {
                        progress: (p) => progress(p, barId)
                    }).then(function(data) {
                        me.addFileToServer({
