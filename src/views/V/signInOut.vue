...
import { SubentryCheckSign, SubentrySignUp, SubentrySignOff } from '@/apis/task'
...
export default {
  ...
  methods: {
    onTaskLoad(taskInfo) {
      //报名
      if (Date.now() < new Date(taskInfo.entryEndTime) && Date.now() < new Date(taskInfo.taskEndTime)) {
