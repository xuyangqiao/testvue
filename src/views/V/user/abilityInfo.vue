<template>
    ...
    <div class="capacity-mesg-wrpper">
        <div class="userinfo-nav">
            <el-button :type="navIndex==1?'sure':'text'" @click="navIndex=1">擅长领域</el-button>
            <el-button :type="navIndex==2?'sure':'text'" @click="navIndex=2">能力认证</el-button>
            <el-button :type="navIndex==3?'sure':'text'" @click="navIndex=3,editWorkTag='index'">作品集</el-button>
        </div>
                    ...
                    <el-form-item v-if="hasEdit">
                        <el-row>
                            <el-col :span="6" :offset="9">
                                <el-button type="sure" class="btn-width-all" @click="saveSpecialty">确定</el-button>
                            </el-col>
                        </el-row>
                    </el-form-item>
                </el-form>
            ...
            <div class="capacity-agree-wrapper" v-if="navIndex==2">
                <div class="capacity-agree-main">
                    <div class="get-capacity">
                        <h4>已获得</h4>
                        <p class="capacity-tips" v-if="abilityList.length==0">暂时还未获得成就，去努力吧！</p>
                    ...
                    <div class="loss-capacity">
                        <h4>未获得</h4>
                        <p class="capacity-tips" v-if="abilityList.length==4">恭喜你，获得所有能力认证！</p>
            ...
            <div class="sample-reels-wrapper" v-if="navIndex==3">
                <div v-if="editWorkTag=='index'">
                    <div class="chose-sample-nav">
                        <el-button :type="workTab=='1'?'sure':'text'" @click="workTab=1">历史作品</el-button>
                        <el-button :type="workTab=='2'?'sure':'text'" @click="workTab=2">平台作品</el-button>
                    </div>
                        ...
                        <div class="history-sample" v-if="workTab==1">
                            <el-row :gutter="12">
                                <el-col :span="8" v-for="(item,i) in workList" :key="i">
                                    <el-button type="danger" @click="delHistory(item)" v-if="workList.length>1">删除</el-button>
                                    <div class="sample-item">
                                        <a href="javascript:;" class="sample-item-a" @click="workForm=item,editWorkTag='info'">
                                            ...
                                            <div class="sample-info">
                                                <p>项目名称：{{item.worksName}}</p>
                                                <p>任务截止时间：{{item.worksEndTime}}</p>
                                            </div>
                                        </a>
                                    </div>
                                </el-col>
                ...
                <div class="sun-sample-info" v-if="editWorkTag=='info'">
                    <div class="chose-sample-nav">
                        <el-button type="sure" @click="editWorkTag='index'">返回&gt;&gt;</el-button>
                    </div>
                ...
                <div class="add-sample-jh" v-if="editWorkTag=='add'" v-loading="loading2">
                        ...
                        <el-row>
                            <el-col :span="10">
                                ...
                                <el-form-item label="完成日期:">
                                    <el-date-picker type="datetime" v-model="workForm.worksEndTime" placeholder="请选择时间" format='yyyy-MM-dd HH:mm:ss' style="width:100%">
                                    </el-date-picker>
                                </el-form-item>
                            </el-col>
                            <el-col :span="24">
                            ...
                            <el-col :span="10" class="update-text">
                                <el-form-item label="附件:">
                                    <el-upload ref="EnclosureFile" class="upload-demo" action="https://jsonplaceholder.typicode.com/posts/" :auto-upload="false" multiple>
                                        <el-button slot="trigger" size="small" type="sure">添加文件</el-button>
                                    </el-upload>
                                </el-form-item>
                            </el-col>
                        </el-row>
                    ...
                    <div class="btn-center-wrap">
                        <el-button slot="trigger" size="small" type="cancle" @click="editWorkTag='index'">取消上传</el-button>
                        <el-button slot="trigger" size="small" type="sure" @click="addWork">确认上传</el-button>
                    </div>
...
<script>
import { AbilityParamV, AbilityListV, SpecialtyV, TsechnologyV, CheckUserInfoV, WorkCollectionV, SaveWorkCollectionV, UpdateWorkCollectionV, delHistory } from '@/apis/person';
...
export default {
    data() {
        return {
            ...
            //能力认证数据
            abilityParamList: {},
            notAbility: ["first-gold", "Skilled-people", "Task-people", "Control-man"],
            abilityList: [],
            //作品集数据
            workTab: 1,
            workList: [],
    ...
    created() {
        ...
        this.hasEdit = this.$route.query.userId == getUser().userId || getUser().userType == 'V';
    ...
    methods: {
        ...
        async getData(id) {
            ...
            if (res.success) {
                ...
                this.checkedRole = userInfo.expertise ? userInfo.expertise.split("/") : []
                this.checkedSoft = userInfo.technology ? userInfo.technology.split("/") : []
            } else {
        ...
        async GetSubTaskList(id) {
            const res = await subTaskList({ appType: "", appAreas: "", packageType: "", state: "7,8", vUserId: id, page: 1, row: 0 })
        ...
        async addWork() {
            ...
            if (res.success) {
                ...
                let file = [], fileListType = [], PromiseList = [];
                ...
                file.map((item, i) => {
                    if (fileListType[i] == 3) {
                        PromiseList.push(uploadFile(item, `/user-info/${getUser().userId}/exhibition`, "abilityDiv" + j++));
                    } else {
                        PromiseList.push(uploadFile(item, `/user-info/${getUser().userId}/exhibition`));
                    }
                });
                Promise.all(PromiseList).then(data => {
                    ...
                    data.map((item, i) => {
                        ...
                        fileList.push({
                            bindid: id,
                            findex: i < ReferencePicture.length ? 'samplereels' : 'enclosure',
        ...
        //跳转到子任务详情页
        toSubTask(item) {
            this.$router.push({ name: 'V_detail', query: { id: item.id, taskId: item.taskId } })
        },
        cancalEdit() {
